...
class vsItem(vsDB):
	...
	def lastChange(self):
		changeQuery = requests.get(self.vs_url + '/API/item/' + self.item_id + '/metadata/changes', auth=self.vs_auth, headers=self.vs_headers)
		changeQuery.encoding = 'utf-8'
		tree = ET.fromstring(changeQuery.text)
